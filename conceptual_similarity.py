...
def parse_word_sim_353(path):
    ...
    with open(path, 'r') as file:
        for line in file.readlines()[1:]:
            ...
            result.append((temp[0], temp[1], float(score))) # result: [ (temp_0, temp_1, score) ... ]
        ...
        for couple_terms in ws353:
            ...
            for s1 in synset1:
                for s2 in synset2:
                    ...
                    maxs.append(metric(s1, s2)) # misura di similarità considerata per ogni senso di ogni couple_terms
            if len(maxs) == 0:  # word without senses (ex.: proper nouns)
                maxs = [-1]
                to_remove.append(j)
            sim_metric.append(max(maxs)) # massime similarità per ogni couple_terms
            j += 1
        similarities.append(sim_metric) # [ [score1_metrica1, score_2,metrica1 ..], [score1_metrica2,...], [score1_metrica3, ..]]
