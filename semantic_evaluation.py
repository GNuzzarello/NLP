...
def get_synset_terms(sense):
    ...
    params = {
        "id": sense,
        "key": "0e1b3e64-3e24-412b-a39d-7e5b2536ed9f",  # my API key        #alternative key: 67adc825-bbcc-4cd4-8d6c-71ecdb875e7c
...
if __name__ == "__main__":
    ...
    with open(path + config["output"] + 'results_task1.tsv', "w", encoding="utf-8") as out:
        ...
        for couple in annotations_1:  # is equal to use annotations_1 or annotations_2, because the words are the same
            ...
            annotations_algorithm.append(((s1, s2), score*4))
            out.write("{0}\t{1}\t{2:.2f}\n".format(key[0], key[1], score*4))
    ...
    # score delle coppie annotate di sensi uniti in un unica lista, una per ogni annotatore
    score_senses_1_couple = []
    score_senses_2_couple = []
    score_senses_3_couple = []

    scores_senses_1_first = list(zip(*senses_1))[2]
    score_senses_1_second = list(zip(*senses_1))[3]
    for couple_s in zip(scores_senses_1_first, score_senses_1_second):
        #print("coppia annotata di sensi")
        #print(couple_s)
        score_senses_1_couple.append(couple_s) #append each time a couple of annotated BN synsets
    ...
    #k_12 = cohen_kappa_score(int_score_human_1, int_score_human_2)
    #k_mean = ( k_12 + k_13 + k_23 ) / 3
    k_1 = 0
    for c in zip(score_senses_1_couple, score_senses_2_couple): # accoppio le 2 diverse annotazioni dei 2 annotatori
